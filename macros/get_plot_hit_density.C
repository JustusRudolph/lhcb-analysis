...
void get_plot_hit_density(unsigned nEvents=2000, unsigned t_res_ps=50, bool plot_only=false) {
  ...
  // number of velo hits per MC particle, with and without electrons in the sample
  TH1D *h_hits_per_track_all = nullptr;
  TH1D *h_hits_per_track_no_electrons = nullptr;
  ...
  if (plot_only) {
    ...
    h_hits_per_track_all = (TH1D*) root_file->Get("hits_per_track_all");
    h_hits_per_track_no_electrons = (TH1D*) root_file->Get("hits_per_track_no_electrons");
    // detach before closing, the file owns whatever is read out of it
    if (h_hit_density) h_hit_density->SetDirectory(0);
    if (h_hits_per_track_all) h_hits_per_track_all->SetDirectory(0);
    if (h_hits_per_track_no_electrons) h_hits_per_track_no_electrons->SetDirectory(0);
    root_file->Close();
  } else {
    ...
    h_hits_per_track_all = new TH1D("hits_per_track_all",
      "Hits per track;N_{hits}^{velo};MC particles", 41, -0.5, 40.5);
    h_hits_per_track_no_electrons = new TH1D("hits_per_track_no_electrons",
      "Hits per track;N_{hits}^{velo};MC particles", 41, -0.5, 40.5);
    h_hits_per_track_all->SetDirectory(0);
    h_hits_per_track_no_electrons->SetDirectory(0);
    h_hits_per_track_all->Sumw2();
    h_hits_per_track_no_electrons->Sumw2();
    for (unsigned i_track = 0; i_track < mc_track_tree->GetEntries(); i_track++) {
      mc_track_tree->GetEntry(i_track);
      h_hits_per_track_all->Fill(n_velo_hits);
      int mc_pid_abs = mc_pid < 0 ? -mc_pid : mc_pid;
      if (mc_pid_abs != 11) h_hits_per_track_no_electrons->Fill(n_velo_hits);  // no e+ or e-
    }
    printf("MC particles: %.0f total, %.0f without electrons\n",
           h_hits_per_track_all->GetEntries(), h_hits_per_track_no_electrons->GetEntries());
    mc_file->Close();

    printf("Saving to file %s...\n", root_output_path.Data());
    TFile *root_file = TFile::Open(root_output_path, "RECREATE");
    h_hit_density->Write();
    h_hits_per_track_all->Write();
    h_hits_per_track_no_electrons->Write();
    printf("h_hit_density->GetEntries() = %f\n", h_hit_density->GetEntries());
    root_file->Close();
    printf("h_hit_density->GetEntries() = %f\n", h_hit_density->GetEntries());
  ...
  // hits per track, drawn as points with error bars rather than filled bars
  if (h_hits_per_track_all == nullptr || h_hits_per_track_no_electrons == nullptr) {
  ...
  h_hits_per_track_all->Scale(1.0 / nEvents);
  h_hits_per_track_no_electrons->Scale(1.0 / nEvents);
  h_hits_per_track_all->SetLineColor(kBlue);
  h_hits_per_track_all->SetMarkerColor(kBlue);
  h_hits_per_track_all->SetMarkerStyle(20);
  h_hits_per_track_all->SetMarkerSize(0.7);
  h_hits_per_track_no_electrons->SetLineColor(kRed);
  h_hits_per_track_no_electrons->SetMarkerColor(kRed);
  h_hits_per_track_no_electrons->SetMarkerStyle(21);
  h_hits_per_track_no_electrons->SetMarkerSize(0.7);
  h_hits_per_track_all->SetTitle("Hits per track;N_{hits}^{velo};MC particles / event");
  h_hits_per_track_all->Draw("E1");
  h_hits_per_track_no_electrons->Draw("E1 SAME");
  TLegend *legend_hits = new TLegend(0.6, 0.7, 0.88, 0.85);
  legend_hits->SetBorderSize(0);
  legend_hits->AddEntry(h_hits_per_track_all, "All particles", "lp");
  legend_hits->AddEntry(h_hits_per_track_no_electrons, "Excluding electrons", "lp");
  legend_hits->Draw();
